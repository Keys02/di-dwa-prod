...
const navItems = [
  ...
  { name: "ALL CATEGORIES", path: "/categories", icon: null },
  { name: "MY ORDER", path: "../MyOrder", icon: FiShoppingCart },
  { name: "TRACK ORDER", path: "/Track", icon: null },
  { name: "PAYMENT", path: "/Payment", icon: null },
  { name: "ABOUT", path: "/About", icon: null },
  { name: "CONTACT", path: "/ContactUs", icon: null },
];
...
const Navbar = () => {
  ...
  const handleSearch = (e) => {
    ...
    // Add your search logic here - you could navigate to a search results page
    // navigate(`/search?q=${searchValue}&category=${selectedCategory}`);
  };
  ...
  return (
    <div className="relative">
              ...
              <SheetContent className="bg-white">
                <div className="flex flex-col gap-6 mt-8">
                  ...
                  {navItems.map((item, idx) => (
                    <Link key={idx} to={item.path} onClick={handleMobileNavClick}>
                      <motion.div
                        whileHover={{ x: 10 }}
                        className={`text-black font-semibold hover:text-green-600 cursor-pointer flex items-center gap-3 p-2 rounded-lg hover:bg-green-50 transition-all ${
                          isActiveRoute(item.path) ? 'bg-green-100 text-green-700' : ''
                        }`}
                      >
                        {item.icon && <item.icon className="w-5 h-5" />}
                        {item.name}
                        {item.name === "MY ORDER" && cartCount > 0 && (
                  ...
                  <div className="border-t pt-4 space-y-3">
                    ...
                    <Link to="/SignUp" onClick={handleMobileNavClick} className="block">
      ...
      >
        {navItems.map((item, idx) => (
          <Link key={idx} to={item.path} className="flex-1">
            <motion.div
              whileHover={{ y: -2, backgroundColor: "#fbbf24" }}
              whileTap={{ scale: 0.95 }}
              className={`px-6 py-4 border-r border-[#46900b] cursor-pointer transition-all duration-200 text-center ${
                isActiveRoute(item.path) ? "bg-yellow-400 text-black" : "hover:text-black"
              }`}
            >
              {item.name === "MY ORDER" ? (
              ...
              <div className="relative h-full">
                ...
                >
                  ...
                  <motion.button
                    ...
                    className="bg-yellow-400 text-black px-8 py-4 rounded-full font-bold text-lg hover:bg-yellow-300 transition-all duration-200 shadow-lg cursor-pointer"
                  >
                    {slide.cta} →
                  </motion.button>
                </motion.div>
