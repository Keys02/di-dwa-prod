...
import { ShoppingCart, Heart, Star, TrendingUp, Grid, List, ChevronRight, Eye, Clock, MessageCircle, Package, ArrowRight } from "lucide-react";
...
// Updated trends with social media links and platforms
const trends = [
  { 
    id: 1, 
    title: "Get to know more by catching up with our latest interviews", 
    image: trend2Img, 
    views: "12.5K", 
    readTime: "5 min",
    platform: "youtube",
    url: "https://youtube.com/@didwa" // Replace with actual YouTube handle
  },
  { 
    id: 2, 
    title: "Nana Akua Amoako - Founder and Team Lead", 
    image: trend3Img, 
    views: "8.2K", 
    readTime: "8 min",
    platform: "instagram",
    url: "https://instagram.com/didwa_gh" // Replace with actual Instagram handle
  },
  { 
    id: 3, 
    title: "Follow and get updates on interviews, advertisements, discounts and more", 
    image: trend4Img, 
    views: "15.7K", 
    readTime: "3 min",
    platform: "tiktok",
    url: "https://tiktok.com/@didwa_gh" // Replace with actual TikTok handle
  },
];
...
function CartModal({ isOpen, onClose, onContinueShopping, onGoToOrder, productName }) {
...
// ProductCard component with built-in cart functionality
...
function ProductSection({ title, products, cartItems, setCartItems, hasPromptedUser, setHasPromptedUser }) {
  ...
  const handleGoToOrder = () => {
    setShowModal(false);
    setHasPromptedUser(true);
    // Navigate to MyOrder page for cart management
    navigate('/MyOrder', { state: { cartItems } });
  };
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-2">
            <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-green-100 text-green-600' : 'text-gray-400 hover:text-gray-600'}`}>
              <Grid className="w-4 h-4" />
            </button>
            <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-green-100 text-green-600' : 'text-gray-400 hover:text-gray-600'}`}>
...
// Updated TrendCard component with social media navigation
function TrendCard({ trend }) {
  ...
  const getPlatformColor = (platform) => {
    switch (platform) {
      ...
      default:
        return 'bg-red-600 hover:bg-green-500';
    }
  };
    ...
    <div className="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden transform hover:-translate-y-1"
      ...
      <div className="relative h-64 bg-gray-100 overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <div className="bg-black/70 rounded-full px-3 py-1 text-white text-xs font-medium flex items-center space-x-1">
            <span>{getPlatformIcon(trend.platform)}</span>
      ...
      <div className="p-4 text-center">
        ...
        <button 
          ...
          className={`w-full ${getPlatformColor(trend.platform)} text-white font-bold px-4 py-2 rounded-lg transition-all duration-300 transform ${isHovered ? 'scale-105' : 'scale-100'} flex items-center justify-center space-x-2`}
        >
          <span>VIEW ON {trend.platform.toUpperCase()}</span>
...
// Updated PromotionalSection with working WhatsApp and Instagram links
function PromotionalSection() {
  const [message, setMessage] = useState('');

  const handleWhatsAppClick = () => {
    const phoneNumber = '233123456789'; // Replace with your actual WhatsApp number
    const encodedMessage = encodeURIComponent(message || 'Hello! I\'m interested in Di-dwa products.');
    const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
    window.open(whatsappUrl, '_blank', 'noopener,noreferrer');
  };

  const handleInstagramClick = () => {
    const instagramUrl = 'https://instagram.com/didwa_gh'; // Replace with your actual Instagram handle
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <button 
                ...
                className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-lg transition-colors duration-200"
              >
                WHATSAPP US
              </button>
              ...
              <button 
                ...
                className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-bold py-3 rounded-lg transition-all duration-200"
              >
                INSTAGRAM
              </button>
            </div>
...
export default function Home() {
  // Local state management for cart (standalone version)
  ...
  return (
    <main className="space-y-10 py-8 px-4 md:px-16 bg-gray-50 min-h-screen">
      <ProductSection 
        ...
        setHasPromptedUser={setHasPromptedUser}
      />
      <ProductSection 
        title="Fruits" 
        products={fruits} 
      ...
      />
      <ProductSection 
        title="Baking & Breakfast" 
        products={bakingAndBreakfast} 
        cartItems={cartItems}
        setCartItems={setCartItems}
        hasPromptedUser={hasPromptedUser}
        setHasPromptedUser={setHasPromptedUser}
