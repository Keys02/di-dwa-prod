...
const Login = () => {
  ...
  const handleSignUpClick = () => {
    navigate('/signup'); // Navigate to SignUp.jsx
  };
  ...
  const handleSocialAuth = (provider) => {
    switch (provider) {
      case 'google':
        // Google OAuth flow
        window.open('https://accounts.google.com/oauth/authorize?client_id=YOUR_GOOGLE_CLIENT_ID&redirect_uri=YOUR_REDIRECT_URI&response_type=code&scope=email profile', '_blank');
        break;
      case 'facebook':
        // Facebook OAuth flow
        window.open('https://www.facebook.com/v18.0/dialog/oauth?client_id=YOUR_FACEBOOK_APP_ID&redirect_uri=YOUR_REDIRECT_URI&scope=email', '_blank');
        break;
      case 'instagram':
        // Instagram Basic Display API flow
        window.open('https://api.instagram.com/oauth/authorize?client_id=YOUR_INSTAGRAM_CLIENT_ID&redirect_uri=YOUR_REDIRECT_URI&scope=user_profile,user_media&response_type=code', '_blank');
        break;
      default:
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Create Account Section - Updated with navigation */}
        <motion.div 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        {/* Social Login - Updated with authentication handlers */}
        <div className="text-center">
          <div className="flex justify-center gap-4">
            {socialButtons.map(({ icon: Icon, color, hoverColor, name, provider }, index) => (
              <motion.button
                key={index}
