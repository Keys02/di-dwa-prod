...
import { CheckCircle, ArrowLeft, Clock, MapPin, Phone, X, Edit3, Plus, Minus, Trash2 } from 'lucide-react';
...
const PaymentPage = () => {
  ...
  useEffect(() => {
    if (location.state?.cartItems) {
      const transformedItems = location.state.cartItems.map(item => {
        // Calculate price based on selection mode
        let itemPrice = 0;
        let itemQuantity = 1;
        let displayUnit = '';
        
        if (item.selectedOption === 'Amount') {
          itemPrice = parseFloat(item.amount) || 0;
          itemQuantity = 1;
          displayUnit = `GH₵${item.amount || '0.00'}`;
        } else {
          itemPrice = item.pricePerUnit || 5.00;
          itemQuantity = parseInt(item.quantity) || 1;
          displayUnit = `${item.quantity} qty`;
        }
        ...
        return {
          ...
          unit: displayUnit,
          originalItem: item // Keep reference to original item structure
        };
      });
      setCurrentCartItems(transformedItems);
    } else {
      // Fallback to empty cart if no items passed
      setCurrentCartItems([]);
    }
  }, [location.state]);
  ...
  const handleTrackOrder = () => {
    if (orderConfirmed) {
      ...
      const orderData = {
        ...
        orderId: 'DD' + Math.random().toString(36).substr(2, 8).toUpperCase(),
      ...
      alert('Please complete your payment first to track your order.');
    }
  };
  ...
  const subtotal = currentCartItems.reduce((sum, item) => {
    if (item.originalItem?.selectedOption === 'Amount') {
      return sum + (parseFloat(item.originalItem.amount) || 0);
    }
  ...
  const handleRemoveItem = (itemId) => {
    setCurrentCartItems(items => items.filter(item => item.id !== itemId));
  };
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg">
          ...
          <button
            ...
            className={`w-full py-4 rounded-xl font-semibold text-white transition-all duration-300 ${
              isProcessing || currentCartItems.length === 0
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg">
          ...
          <div className="mb-6">
            <button
              ...
              className={`w-full flex items-center justify-between p-4 border-2 border-dashed rounded-xl hover:border-green-400 hover:bg-green-50 transition-all duration-200 ${
                deliveryAddress ? 'border-green-300 bg-green-50' : 'border-gray-300'
              }`}
            >
              <div className="flex items-center gap-3">
              ...
              <div className="text-sm text-gray-500">
                {deliveryAddress ? 'Edit' : 'Required'}
              </div>
            </button>
            
            {/* Delivery Address Form */}
            {showDeliveryForm && (
              <div className="mt-4 p-4 border border-gray-200 rounded-xl bg-gray-50 animate-slide-in">
                <div className="mb-3">
                  ...
                  <textarea
                    ...
                    placeholder="Enter your full delivery address (e.g., Accra, Legon Campus, Akuafo Hall, Room 123)"
                ...
                {deliveryAddress && (
                  <div className="mb-4">
                    ...
                    <div className="text-xs text-gray-500 mt-1 bg-yellow-50 p-2 rounded border-l-2 border-yellow-300">
          ...
          <div className="space-y-4 mb-6">
            {currentCartItems.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-400">Add some items to continue with checkout</p>
              </div>
            ) : (
              currentCartItems.map((item) => (
                <div key={item.id} className="flex items-center gap-4 p-3 hover:bg-gray-50 rounded-lg transition-colors border border-gray-100">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-semibold text-green-600">
                      {item.originalItem?.selectedOption === 'Amount' 
                        ? `Total: Gh₵${parseFloat(item.originalItem.amount || 0).toFixed(2)}`
                  ...
                  <div className="text-right min-w-0">
                    <p className="font-semibold">
                      Gh₵{item.originalItem?.selectedOption === 'Amount' 
                        ? parseFloat(item.originalItem.amount || 0).toFixed(2)
